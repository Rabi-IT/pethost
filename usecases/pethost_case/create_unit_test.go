...
func makeSut(g pethost_gateway.PetHostGateway) *pethost_case.PetHostCase {
	return pethost_case.New(g)
}

func Test_Unit_Create__should_fail_if_required_fields_are_empty(t *testing.T) {
	sut := makeSut(nil)

	_, err := sut.Create(context.Background(), &pethost_case.CreateInput{
		Name:           "",
		TaxID:          "",
		City:           "",
		State:          "",
...
func Test_Unit_Create__should_not_fail_if_all_optional_fields_are_not_filled_in(t *testing.T) {
	gateway := mocks.NewPetHostGateway(t)
	expectedID := "ANY_ID"
	gateway.On("Create", mock.Anything).Return(expectedID, nil)
	sut := pethost_case.New(gateway)

	id, err := sut.Create(context.Background(), &pethost_case.CreateInput{
		Name:           "Name",
		TaxID:          "TaxID",
		City:           "City",
		State:          "State",
