...
func makeSut(g tutor_gateway.TutorGateway) *tutor_case.TutorCase {
	return tutor_case.New(g)
}

func Test_Unit_Create__should_fail_if_required_fields_are_empty(t *testing.T) {
	DUMMY_GATEWAY := mocks.NewTutorGateway(t)
	sut := makeSut(DUMMY_GATEWAY)

	_, err := sut.Create(context.Background(), &tutor_case.CreateInput{
		Name:           "",
		Photo:          "Photo",
		TaxID:          "",
		City:           "",
...
func Test_Unit_Create__should_not_fail_if_all_optional_fields_are_not_filled_in(t *testing.T) {
	gateway := mocks.NewTutorGateway(t)
	expectedID := "ANY_ID"
	gateway.On("Create", mock.Anything).Return(expectedID, nil)
	sut := tutor_case.New(gateway)

	id, err := sut.Create(context.Background(), &tutor_case.CreateInput{
		Name:           "Name",
		TaxID:          "TaxID",
		City:           "City",
		State:          "State",
